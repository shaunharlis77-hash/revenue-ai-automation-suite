...
from app.services.proposal import WORKFLOW_NAME, draft_proposal  # noqa: E402
...
def main() -> int:
    reset_workflow_runs()
    inputs = load_json(REPO_ROOT / "sample-data" / "proposal-inputs.json")
    expected_results = load_json(
        REPO_ROOT / "sample-data" / "proposal-expected-results.json"
    )
    expected_by_id = {result["proposal_id"]: result for result in expected_results}

    passed = 0
    failed = 0

    for input_data in inputs:
        request = ProposalDraftRequest(**input_data)
        result = draft_proposal(request)
        expected = expected_by_id[result.proposal_id]
        failures = checked_field_failures(result, expected)

        if failures:
            failed += 1
            print(f"FAIL {result.proposal_id}:")
            for failure in failures:
                print(f"  - {failure}")
        else:
            passed += 1
            print(
                f"PASS {result.proposal_id}: "
...
def checked_field_failures(result, expected: dict) -> list[str]:
    ...
    if expected["risk_notes_required"] and not result.risk_notes:
        failures.append("risk_notes: expected at least one risk note")

    if not result.proposal_title.strip():
        failures.append("proposal_title: expected non-empty title")
    if not result.executive_summary.strip():
        failures.append("executive_summary: expected non-empty summary")
    if not result.recommended_package.strip():
        failures.append("recommended_package: expected non-empty recommendation")
    if not result.scope_items:
        failures.append("scope_items: expected at least one item")
    ...
    generated_text = " ".join(
        [
            ...
            " ".join(result.scope_items),
            " ".join(result.implementation_considerations),
    ...
    for banned_phrase in BANNED_PROMISE_LANGUAGE:
        if banned_phrase in generated_text:
            failures.append(f"generated text contains banned phrase {banned_phrase!r}")
